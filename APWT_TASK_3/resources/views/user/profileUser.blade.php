...
@section('contentUsers')
    ...
    <br><br>
     <h1>Profile</h1>
     <form action= "{{route('profileEdit')}}" class "form-group" method = "post">
        {{csrf_field()}}
        ...
        <div class="form-group p-1">
            ...
            <input type="radio" name="gender" value="{{$user->male}}">Male
            <input type="radio" name="gender" value="{{$user->female}}">Female
            <input type="radio" name="gender" value="{{$user->other}}">Other
            @error('gender')
                <span class="text-danger">{{$message}}</span>
            @enderror
        </div>
